...
>(({ className, inset, children, ...props }, ref) => {
    ...
    return (
        <TextClassContext.Provider
            value={cn(
                'select-none text-sm native:text-lg text-white',
        ...
        >
            <DropdownMenuPrimitive.SubTrigger
                ref={ref}
                className={cn(
                    'web:focus:bg-accent web:hover:bg-accent active:bg-accent native:py-2 flex flex-row items-center gap-2 rounded-sm px-2 py-1.5 web:cursor-default web:select-none web:outline-none',
                    open && 'bg-accent',
                    inset && 'pl-8',
                    className,
                )}
...
>(({ className, inset, ...props }, ref) => (
    <TextClassContext.Provider value='select-none text-sm native:text-lg text-popover-foreground web:group-focus:text-accent-foreground'>
        <DropdownMenuPrimitive.Item
            ref={ref}
            className={cn(
                'native:py-2 web:focus:bg-accent active:bg-accent web:hover:bg-accent group relative flex flex-row items-center gap-2 rounded-sm px-2 py-1.5 web:cursor-default web:outline-none',
                inset && 'pl-8',
...
>(({ className, children, checked, ...props }, ref) => (
    <DropdownMenuPrimitive.CheckboxItem
        ref={ref}
        className={cn(
            'web:group native:py-2 web:focus:bg-accent active:bg-accent relative flex flex-row items-center rounded-sm py-1.5 pl-8 pr-2 web:cursor-default web:outline-none',
            props.disabled && 'opacity-50 web:pointer-events-none',
            className,
        )}
        checked={checked}
...
>(({ className, inset, ...props }, ref) => (
    <DropdownMenuPrimitive.Label
        ref={ref}
        className={cn(
            'native:text-base text-foreground px-2 py-1.5 text-sm font-semibold web:cursor-default',
            inset && 'pl-8',
            className,
        )}
        {...props}
